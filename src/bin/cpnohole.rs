use std::{
    env, fs::{self, OpenOptions}, io::Result, os::unix::fs::FileExt, path::Path
};
...
fn main() {
    ...
    if args.len() != ARGS_COUNT {
        eprintln!("❗️arguments count error, except two found {}", args.len());
        panic!()
    }
...
fn make_hole_file(path: &Path, buffer: &[u8]) -> Result<()> {
    let file: std::fs::File = OpenOptions::new().write(true).create(true).truncate(true).open(path)?;
